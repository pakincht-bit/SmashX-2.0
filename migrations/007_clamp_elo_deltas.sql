-- ============================================
-- RPC: record_match_result (ELO with clamped deltas)
-- Clamps per-player deltas to +15..+40 (wins) and -40..-15 (losses).
-- Must stay in sync with ELO_* constants in utils.ts.
-- ============================================
...
BEGIN
  ...
  -- 4. Apply ELO to each winner: delta clamped to [15, 40]
  FOR v_rec IN
  ...
  LOOP
    ...
    v_delta := GREATEST(15, LEAST(40, v_delta));
  ...
  -- 5. Apply ELO to each loser: delta clamped to [-40, -15]
  FOR v_rec IN
  ...
  LOOP
    ...
    v_delta := LEAST(-15, GREATEST(-40, v_delta));
    v_new_points := v_rec.points + v_delta;
