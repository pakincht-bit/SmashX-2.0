...
import { Calendar, Trophy, Activity, User as UserIcon } from 'lucide-react';
import { User } from '../types';
import { getAvatarColor, triggerHaptic } from '../utils';

interface BottomNavProps {
 activeTab: 'sessions' | 'leaderboard' | 'stats';
 onTabChange: (tab: 'sessions' | 'leaderboard' | 'stats') => void;
...
const BottomNav: React.FC<BottomNavProps> = React.memo(({ activeTab, onTabChange, currentUser }) => {
 ...
 <div
 className="absolute top-0 bottom-0 left-0 w-[33.333%] p-1.5 transition-all duration-500 cubic-bezier(0.68, -0.6, 0.32, 1.6)"
